...
const RoomCard = ({ room }) => {
  ...
  return (
   <div >
     <Card className="bg-base-100 w-102 shadow-sm p-0 group">
      ...
      <div className="card-body p-4 space-y-4">
        ...
        <div className="card-actions justify-end flex-wrap gap-2">
          {(() => {
            ...
            else if (typeof amenities === 'string' && amenities.trim()) amenitiesList = amenities.split(',').map(s=>s.trim());
            const visible = amenitiesList.slice(0, 3);
            const extra = Math.max(0, amenitiesList.length - 3);
            return (
              <>
                {visible.map((a) => (
                  <div key={a} className="badge badge-outline text-sm px-3 py-1 mr-2">{a}</div>
                ))}
                {extra > 0 && (
                  <div className="badge badge-outline text-sm px-3 py-1">+{extra}</div>
                )}
              </>
            );
          })()}
        ...
        <Button  variant="outline" className={"bg-[#ebebec] hover:bg-[#4F5A2A] hover:text-white w-full"}>View Details</Button>
        </Link>
      </div>
    </Card>
   </div>
